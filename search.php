...
<div class="box-container" id="content">
    ...
    INNER JOIN markets ON markets_products.market_id = markets.market_id) INNER JOIN images ON products.image_id = images.image_id) WHERE product_name LIKE '%$q%' OR market_name LIKE '%$q%';";
    $result = mysqli_query($link, $query);
    if($result){
        while($row = mysqli_fetch_assoc($result)){
            $imageURL = 'image/products/'.substr($row['file_name'],8);
            echo '<div class="box" style="height:50rem">';
            echo '<img src="'.$imageURL.'" alt="'.$row['product_name'].' image">
            <h3>'.$row['product_name'].'</h3>
            <p>'.$row['description'].'</p>
            <p><b>'.$row['market_name'].'</b></p>
